...
def backtest_supertrend(df, atr_length, factor, buffer_multiplier, hard_stop_distance):
    ...
    try:
        ...
        if trade_count > 0:
            ...
            sharpe_ratio = np.sqrt(252) * (returns_series.mean() / returns_series.std()) if len(running_pnl) > 1 else 0
...
def main():
    ...
    try:
        ...
        # Step 4: Get ranking thresholds from user
        print("\nEnter filtering criteria for parameter ranking:")
        max_drawdown_threshold = get_float_input(
            "Maximum allowed drawdown (as decimal, e.g., 0.30 for 30%): ",
            min_value=0.0,
            max_value=1.0
        )
        min_profit_threshold = get_float_input(
            "Minimum required profit (as decimal, e.g., 0.10 for 10%): ",
            min_value=0.0
        )
                ...
                if all_results:
                    ...
                    if not final_results_df.empty:
                        ...
                        if not top_combinations.empty:
                            ...
                            results_manager.save_top_combinations_trades(top_combinations, df)
                            
                            # Create visualizations for top combinations
                            results_manager.create_performance_visualizations(top_combinations)
