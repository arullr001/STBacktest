...
CURRENT_UTC = "2025-06-12 23:35:45"
CURRENT_USER = "arullr001"

CURRENT_UTC = "2025-06-12 23:54:26"
CURRENT_USER = "arullr001"
...
class BatchProcessor:
    ...
    def __init__(self, directory_manager, memory_manager, gpu_manager=None, hdd_manager=None):
    ...
    def process_batch(self, df, param_combinations, batch_start, batch_size, max_workers=4):
        ...
        # Split processing between GPUs if available
        if self.gpu_manager and self.gpu_manager.gpus:
            try:
                # Calculate GPU split
                nvidia_ratio = 0.75  # 75% to NVIDIA
                nvidia_batch_size = int(adjusted_batch_size * nvidia_ratio)
                intel_batch_size = adjusted_batch_size - nvidia_batch_size

                print(f"NVIDIA batch size: {nvidia_batch_size}")
                print(f"Intel batch size: {intel_batch_size}")

                batch_end = min(batch_start + adjusted_batch_size, len(param_combinations))
                nvidia_combinations = param_combinations[batch_start:batch_start + nvidia_batch_size]
                intel_combinations = param_combinations[batch_start + nvidia_batch_size:batch_end]
            except Exception as e:
                print(f"Error setting up GPU processing: {e}")
                # Fallback to single batch
                batch_end = min(batch_start + adjusted_batch_size, len(param_combinations))
                nvidia_combinations = param_combinations[batch_start:batch_end]
                intel_combinations = []
        else:
            # No GPU available, process as single batch
            batch_end = min(batch_start + adjusted_batch_size, len(param_combinations))
            nvidia_combinations = param_combinations[batch_start:batch_end]
            intel_combinations = []
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Submit tasks based on GPU availability
            if self.gpu_manager and self.gpu_manager.gpus:
                if nvidia_combinations:
                ...
                # Process without GPU splitting
                for combo in param_combinations[batch_start:batch_end]:
                    tasks.append(executor.submit(
                        process_param_combo,
                        (df.copy(), *combo)
                    ))
                ...
                for future in concurrent.futures.as_completed(tasks):
                    try:
                        ...
                        if isinstance(result, list):
                            # Results from GPU batch
                            valid_results = [r for r in result if r and r.get('trade_count', 0) > 0]
                            results.extend(valid_results)
                        elif result and result.get('trade_count', 0) > 0:
                            # Single result
...
CURRENT_UTC = "2025-06-12 22:25:43"
CURRENT_USER = "arullr001"

CURRENT_UTC = "2025-06-12 23:38:54"
CURRENT_USER = "arullr001"

CURRENT_UTC = "2025-06-12 23:57:09"
CURRENT_USER = "arullr001"
...
def main():
    ...
    try:
        ...
        # Initialize batch processor with available managers
            ...
            try:
                ...
                for batch_start in range(0, len(param_combinations), batch_size):
                    ...
                    if batch_results:
                        ...
                        if valid_results:
                            ...
                            print(f"Valid results in this batch: {len(valid_results)}")
                        else:
                ...
                if all_results:
                    ...
                    if not final_results_df.empty:
                        ...
                        required_columns = ['max_drawdown', 'sharpe_ratio', 'win_rate', 'profit_factor']
                        missing_columns = [col for col in required_columns if col not in final_results_df.columns]
                        
                        if missing_columns:
                            print(f"\nWarning: Missing columns in results: {missing_columns}")
                            # Add missing columns with default values
                            for col in missing_columns:
                                final_results_df[col] = 0.0
                        ...
                        # Rank parameter combinations and get top 5
                        ...
                        if not top_combinations.empty:
                            ...
                            print(top_combinations[['atr_length', 'factor', 'buffer_multiplier', 
                                                 'hard_stop_distance', 'total_profit', 'win_rate']].to_string())
                        else:
                            print("\nNo combinations met the ranking criteria")
                    else:
                    ...
                    save_empty_results_file(
                        ...
                        CURRENT_USER,
                        len(param_combinations),
                        reason="No parameter combinations generated profitable trades."
                    )
        ...
        print(f"Started at: {CURRENT_UTC}")
