...
class GridSearchOptimizer:
    ...
    def _run_threaded(self, df: pd.DataFrame, param_combinations: List[Tuple], 
                    optimization_metric: str, min_trades: int, time_exit_hours: float) -> List[Dict]:
        """Run optimization using threading"""
        self._log('info', f"Running optimization with ThreadPoolExecutor using {CPU_THREADS} threads")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=CPU_THREADS) as executor:
            ...
            for params in param_combinations:
                ...
                future = executor.submit(
                    ...
                    df, atr_length, factor, buffer, stop, min_trades, time_exit_hours
                )
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                try:
                    ...
                    self._log('error', f"Error evaluating {params}: {str(e)}")
                
                if i % 10 == 0:
                    self._log('debug', f"Progress: {i}/{len(futures)} ({i/len(futures)*100:.1f}%)")
    ...
    def _run_multiprocess(self, df: pd.DataFrame, param_combinations: List[Tuple], 
        ...
        # Calculate how much memory each worker will need
        memory_per_worker = sys.getsizeof(df) * 2  # Rough estimate
        
        # Calculate safe number of workers based on available memory
        available_ram = psutil.virtual_memory().available
        safe_workers = max(1, int(available_ram * 0.8 / memory_per_worker))
        num_workers = min(CPU_THREADS, safe_workers)
        
        self._log('info', f"Running optimization with ProcessPoolExecutor using {num_workers} workers")
        self._log('debug', f"Memory per worker: {memory_per_worker/1e6:.1f} MB, Available RAM: {available_ram/1e6:.1f} MB")
        
        # Prepare shared data (avoid serializing large DataFrame multiple times)
        # For this, we'll use a temporary file to store the DataFrame
        ...
        try:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
                ...
                for params in param_combinations:
                    ...
                    futures.append(
                        executor.submit(
                            self._evaluate_combination_mp, 
                            temp_path, atr_length, factor, buffer, stop, min_trades, time_exit_hours
                        )
                    )
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures)):
                    try:
                        ...
                        self._log('error', f"Error in multiprocessing evaluation: {str(e)}")
    ...
    def _run_dask(self, df: pd.DataFrame, param_combinations: List[Tuple], 
        ...
        if not HAS_DASK:
            self._log('error', "Dask not available")
        ...
        dask_df = dd.from_pandas(df, npartitions=CPU_THREADS)
        
        # Define a task that can run remotely
        def evaluate_task(params):
            atr_length, factor, buffer, stop = params
            # Note: This assumes SuperTrend implementation is available in workers
            try:
                backtester = Backtester()  # No log manager in worker
                result = backtester.run_backtest(
                    dask_df.compute(),  # Convert back to pandas DataFrame
                    atr_length, factor, buffer, stop,
                    time_exit_hours
                )
                ...
                # Return only what we need to minimize data transfer
                return {
                    'parameters': {
                        'atr_length': atr_length,
                        'factor': factor,
                        'buffer_multiplier': buffer,
                        'hard_stop_distance': stop
                    },
                    'performance': result['performance'],
                    'trade_count': trade_count
                }
            except Exception as e:
                print(f"Error in Dask worker: {str(e)}")
                return None
        
        # Create Dask bag from parameter combinations
        bag = db.from_sequence(param_combinations)
        
        # Map the evaluation function to each parameter set
        result_bag = bag.map(evaluate_task)
        
        # Filter out None results
        valid_results = result_bag.filter(lambda x: x is not None)
        
        # Compute results with progress bar
        with ProgressBar():
            results = valid_results.compute()
        
        return results
    
    def _evaluate_combination(self, df: pd.DataFrame, atr_length: int, factor: float, 
                            buffer: float, stop: float, min_trades: int, time_exit_hours: float) -> Dict:
        """Evaluate a single parameter combination"""
        try:
            # Create a SuperTrend instance for this evaluation
            supertrend = SuperTrend()
            
            # Calculate SuperTrend
            df_st = supertrend.calculate(df, atr_length, factor, buffer)
            
            # Process signals to get trades
            trade_processor = TradeProcessor()
            trades = trade_processor.process_supertrend_signals(df_st, stop, time_exit_hours)
            ...
            self._log('error', f"Error evaluating combination {atr_length}/{factor}/{buffer}/{stop}: {str(e)}")
            return None
    
    @staticmethod
    def _evaluate_combination_mp(df_path: str, atr_length: int, factor: float, 
                               buffer: float, stop: float, min_trades: int, time_exit_hours: float) -> Dict:
        """Evaluate a single parameter combination (multiprocessing version)"""
        try:
            ...
            print(f"Error in multiprocessing evaluation: {str(e)}")
            return None
    ...
    def optimize(self, df: pd.DataFrame, 
        ...
        for gen, ind in enumerate(history):
            generation_stats.append({
                ...
                'parameters': {param['name']: ind['individual'][i] for i, param in enumerate(param_definitions)}
            })
...
class ResultsStorage:
    ...
    def __init__(self, base_dir: str = None, log_manager: LogManager = None):
        self.log = log_manager
        self.current_utc = "2025-06-24 13:21:57"  # Updated timestamp
        self.current_user = "arullr001"           # Updated username
    ...
    def save_backtest_results(self, results: Dict[str, Any], name: str = None) -> str:
        ...
        serializable_results['_metadata'] = {
            'saved_at': self.current_utc,
            'saved_by': self.current_user,
            'filename': filename
        }
...
class ReportGenerator:
    ...
    def generate_chart_optimization(self, results: Dict[str, Any]) -> plt.Figure:
        ...
        for param in ['atr_length', 'factor', 'buffer_multiplier', 'hard_stop_distance']:
            if param in param_analysis:
                boxplot_data.append([
                    param_analysis[param].get('min', 0),
                    param_analysis[param].get('mean', 0) - param_analysis[param].get('std', 0),
                    param_analysis[param].get('median', 0),
                    param_analysis[param].get('mean', 0) + param_analysis[param].get('std', 0),
                    param_analysis[param].get('max', 0)
                ])
                param_names.append(param)
        
        if boxplot_data:
            ax3.boxplot(boxplot_data, labels=param_names, patch_artist=True, 
                      boxprops=dict(facecolor='lightblue'),
                      medianprops=dict(color='red'))
            ax3.set_title('Parameter Distributions (Min, Q1, Median, Q3, Max)')
            ...
            if 'optimal_combination' in param_analysis:
                ...
                for i, param in enumerate(param_names):
                    if param in optimal:
                        ax3.scatter(i+1, optimal[param], color='green', marker='*', s=200, zorder=10)
        else:
            ax3.text(0.5, 0.5, "Parameter distribution data not available", ha='center', va='center', transform=ax3.transAxes)
        
        # Add title with metadata
        optimizer_type = results.get('metadata', {}).get('optimizer_type', 'Unknown')
        metric = results.get('metadata', {}).get('optimization_metric', 'Unknown')
        
        fig.suptitle(f"{optimizer_type.capitalize()} Optimization Results - Targeting {metric}", fontsize=16)
        
        # Add insights as text
        if 'insights' in param_analysis:
            insights = param_analysis['insights']
            if insights and isinstance(insights, list):
                insight_text = "\n".join(f"• {insight}" for insight in insights[:3])  # Show top 3 insights
                fig.text(0.5, 0.01, insight_text, ha='center', fontsize=10, bbox=dict(facecolor='white', alpha=0.5))
    ...
    def _generate_optimization_html_report(self, results: Dict[str, Any]) -> str:
            ...
            <div class="container">
                ...
                <div class="section">
                    ...
                    <table>
                        <tr>
                            ...
                            <td>{meta.get('optimization_metric', 'N/A')}</td>
                        </tr>
                        <tr>
                            <td>Total Parameter Combinations</td>
                            <td>{meta.get('total_combinations', 'N/A')}</td>
                        </tr>
                        <tr>
                            <td>Valid Combinations</td>
                            <td>{meta.get('valid_combinations', 'N/A')}</td>
                        </tr>
                        <tr>
                ...
                <div class="section">
                    ...
                    <table>
                        ...
                        <tr>
                            <td>ATR Length</td>
                            <td>{best_params.get('atr_length', 'N/A')}</td>
                        </tr>
                        <tr>
                            <td>Factor</td>
                            <td>{best_params.get('factor', 'N/A')}</td>
                        </tr>
                        <tr>
                            <td>Buffer Multiplier</td>
                            <td>{best_params.get('buffer_multiplier', 'N/A')}</td>
                        </tr>
                        <tr>
                            <td>Hard Stop Distance</td>
                            <td>{best_params.get('hard_stop_distance', 'N/A')}</td>
                        </tr>
                    </table>
                    
                    <h3>Performance with Best Parameters</h3>
                    <div class="trade-summary">
                        <div class="metric-box">
                            <div class="metric-label">Total Trades</div>
                            <div class="metric-value">{best_performance.get('trade_count', 0)}</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-label">Win Rate</div>
                            <div class="metric-value">{best_performance.get('win_rate', 0)*100:.2f}%</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-label">Profit Factor</div>
                            <div class="metric-value">{best_performance.get('profit_factor', 0):.2f}</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-label">Total Return</div>
                            <div class="metric-value">{best_performance.get('total_profit_pct', 0):.2f}%</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-label">Max Drawdown</div>
                            <div class="metric-value">{best_performance.get('max_drawdown', 0):.2f}%</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-label">Sharpe Ratio</div>
                            <div class="metric-value">{best_performance.get('sharpe_ratio', 0):.2f}</div>
                        </div>
                    </div>
                </div>
        """
        ...
        if param_analysis:
            ...
            if 'optimal_combination' in param_analysis:
                html += """
                    <h3>Recommended Optimal Parameter Combination</h3>
                    <table>
                        <tr>
                            <th>Parameter</th>
                            <th>Optimal Value</th>
                ...
                if param in param_analysis:
                    ...
                    html += f"""
                        <tr>
                            ...
                            <td>{stats.get('most_common', 'N/A')}</td>
                            <td>{importance:.2f}</td>
            ...
            for i, result in enumerate(sorted_results[:20]):
                ...
                html += f"""
                    <tr>
                        ...
                        <td>{perf.get('win_rate', 0)*100:.2f}%</td>
                        <td>{perf.get('profit_factor', 0):.2f}</td>
                        <td>{perf.get('total_profit_pct', 0):.2f}%</td>
                    </tr>
                """
...
"""
        ...
        if all_results:
            ...
            for i, result in enumerate(sorted_results[:10]):
                ...
                markdown += f"| {i+1} | {params.get('atr_length', 'N/A')} | {params.get('factor', 'N/A')} | {params.get('buffer_multiplier', 'N/A')} | {params.get('hard_stop_distance', 'N/A')} | {perf.get('trade_count', 0)} | {perf.get('win_rate', 0)*100:.2f}% | {perf.get('profit_factor', 0):.2f} | {perf.get('total_profit_pct', 0):.2f}% |\n"
...
"""
        ...
        # Add parameter analysis if available
        if param_analysis and 'insights' in param_analysis:
            text += """
-----------------------------------------------------------------------
KEY INSIGHTS
-----------------------------------------------------------------------
"""
        ...
        # Add optimal parameters
        if param_analysis and 'optimal_combination' in param_analysis:
            text += """
-----------------------------------------------------------------------
RECOMMENDED OPTIMAL PARAMETER COMBINATION
-----------------------------------------------------------------------
"""
            optimal = param_analysis['optimal_combination']
            for param, value in optimal.items():
        ...
        if all_results:
            text += f"""
-----------------------------------------------------------------------
TOP PARAMETER COMBINATIONS (Top {min(5, len(all_results))} of {len(all_results)})
-----------------------------------------------------------------------
Rank | ATR | Factor | Buffer | Stop | Trades | Win Rate | P.Factor | Return 
"""
            ...
            # Show top 5 results for text report (keep it simple)
            for i, result in enumerate(sorted_results[:5]):
                params = result['parameters']
                perf = result['performance']
                
                text += f"{i+1:4} | {params.get('atr_length', 'N/A'):3} | {params.get('factor', 'N/A'):6.2f} | {params.get('buffer_multiplier', 'N/A'):6.2f} | {params.get('hard_stop_distance', 'N/A'):4} | {perf.get('trade_count', 0):6} | {perf.get('win_rate', 0)*100:8.2f}% | {perf.get('profit_factor', 0):8.2f} | {perf.get('total_profit_pct', 0):6.2f}%\n"
...
class SuperTrendGUI(QMainWindow):
    """
    Main GUI window for SuperTrend backtester application
    """
    ...
    def __init__(self, log_manager: LogManager = None):
        ...
        self.log = log_manager
        self.current_utc = "2025-06-24 13:37:06"  # Updated timestamp
        self.current_user = "arullr001"           # Updated username
        
        # Initialize components
        self.data_processor = DataLoader(log_manager)
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle(f"SuperTrend Backtester v{APP_VERSION}")
        self.setGeometry(100, 100, 1200, 800)
        self.setWindowIcon(QIcon('icon.png'))  # You would need to add an icon file
    ...
    def setup_optimization_tab(self):
        ...
        # ATR Length range
        ranges_layout.addWidget(QLabel("ATR Length:"), 0, 0)
        self.atr_min_input = QSpinBox()
        self.atr_min_input.setRange(1, 100)
        self.atr_min_input.setValue(10)
        ranges_layout.addWidget(self.atr_min_input, 0, 1)
        
        ranges_layout.addWidget(QLabel("to"), 0, 2)
        self.atr_max_input = QSpinBox()
        self.atr_max_input.setRange(1, 100)
        self.atr_max_input.setValue(20)
        ranges_layout.addWidget(self.atr_max_input, 0, 3)
        
        ranges_layout.addWidget(QLabel("Step:"), 0, 4)
        self.atr_step_input = QSpinBox()
        self.atr_step_input.setRange(1, 10)
        self.atr_step_input.setValue(2)
        ranges_layout.addWidget(self.atr_step_input, 0, 5)
        ...
        self.factor_min_input.setValue(1.0)
        ranges_layout.addWidget(self.factor_min_input, 1, 1)
        ...
        self.factor_max_input.setValue(5.0)
        ranges_layout.addWidget(self.factor_max_input, 1, 3)
        
        ranges_layout.addWidget(QLabel("Step:"), 1, 4)
        self.factor_step_input = QDoubleSpinBox()
        self.factor_step_input.setRange(0.1, 1.0)
        self.factor_step_input.setSingleStep(0.1)
        self.factor_step_input.setValue(0.5)
        ranges_layout.addWidget(self.factor_step_input, 1, 5)
        
        # Buffer range
        ranges_layout.addWidget(QLabel("Buffer:"), 2, 0)
        self.buffer_min_input = QDoubleSpinBox()
        self.buffer_min_input.setRange(0.1, 3.0)
        self.buffer_min_input.setSingleStep(0.1)
        self.buffer_min_input.setValue(0.1)
        ranges_layout.addWidget(self.buffer_min_input, 2, 1)
        ...
        ranges_layout.addWidget(QLabel("Step:"), 2, 4)
        self.buffer_step_input = QDoubleSpinBox()
        self.buffer_step_input.setRange(0.05, 0.5)
        self.buffer_step_input.setSingleStep(0.05)
        self.buffer_step_input.setValue(0.1)
        ranges_layout.addWidget(self.buffer_step_input, 2, 5)
        
        # Stop range
        ranges_layout.addWidget(QLabel("Stop:"), 3, 0)
        self.stop_min_input = QSpinBox()
        self.stop_min_input.setRange(5, 1000)
        self.stop_min_input.setValue(20)
        ranges_layout.addWidget(self.stop_min_input, 3, 1)
        
        ranges_layout.addWidget(QLabel("to"), 3, 2)
        self.stop_max_input = QSpinBox()
        self.stop_max_input.setRange(5, 1000)
        self.stop_max_input.setValue(50)
        ranges_layout.addWidget(self.stop_max_input, 3, 3)
        
        ranges_layout.addWidget(QLabel("Step:"), 3, 4)
        self.stop_step_input = QSpinBox()
        self.stop_step_input.setRange(5, 50)
        self.stop_step_input.setValue(10)
        ranges_layout.addWidget(self.stop_step_input, 3, 5)
        ...
        # Optimization metric
        settings_layout.addWidget(QLabel("Optimization Metric:"), 0, 0)
        self.metric_combo = QComboBox()
        self.metric_combo.addItems([
            "profit_factor", "win_rate", "risk_adjusted_return", 
            "sharpe_ratio", "sortino_ratio", "calmar_ratio"
        ])
        settings_layout.addWidget(self.metric_combo, 0, 1)
        ...
        # Optimization method
        settings_layout.addWidget(QLabel("Method:"), 2, 0)
        ...
        # Parallelism
        settings_layout.addWidget(QLabel("Parallelism:"), 3, 0)
        self.parallelism_combo = QComboBox()
        self.parallelism_combo.addItems([
            "thread", "process", "dask", "none"
        ])
        settings_layout.addWidget(self.parallelism_combo, 3, 1)
        ...
        run_layout.addWidget(self.optimization_progress)
        ...
        results_layout.addWidget(self.optimization_results_text)
...
"""
    ...
    def on_run_optimization(self):
        ...
        try:
            ...
            if method == "Grid Search":
                # Create optimizer
                optimizer = GridSearchOptimizer(self.log)
                
                # Run optimization
                progress.setLabelText(f"Grid search with {total_combinations} combinations...")
                progress.setValue(20)
                QApplication.processEvents()
                ...
                self.optimization_results = optimizer.optimize(
                    ...
                    optimization_metric=optimization_metric,
                    min_trades=min_trades,
                    time_exit_hours=self.time_exit_input.value(),
                    parallelism=parallelism
                )
                
            elif method == "Bayesian Optimization":
                # Create optimizer
                optimizer = BayesianOptimizer(self.log)
                
                # Convert ranges for Bayesian optimization
                bayesian_ranges = {
                    'atr_length': (min(param_ranges['atr_lengths']), max(param_ranges['atr_lengths'])),
                    'factor': (min(param_ranges['factors']), max(param_ranges['factors'])),
                    'buffer': (min(param_ranges['buffers']), max(param_ranges['buffers'])),
                    'stop': (min(param_ranges['stops']), max(param_ranges['stops']))
                }
                
                # Run optimization
                progress.setLabelText(f"Bayesian optimization...")
                progress.setValue(20)
                QApplication.processEvents()
                ...
                # Create optimizer
                optimizer = GeneticOptimizer(self.log)
                
                # Convert ranges for genetic algorithm
                genetic_ranges = {
                    'atr_length': (min(param_ranges['atr_lengths']), max(param_ranges['atr_lengths'])),
                    'factor': (min(param_ranges['factors']), max(param_ranges['factors'])),
                    'buffer': (min(param_ranges['buffers']), max(param_ranges['buffers'])),
                ...
                # Run optimization
                progress.setLabelText(f"Genetic algorithm optimization...")
                progress.setValue(20)
                QApplication.processEvents()
            ...
            self.status_bar.showMessage(f"Optimization completed")
            self._log('info', f"Optimization completed")
            
            # Save results
            confirm_save = QMessageBox.question(
                self,
                "Save Results",
                "Do you want to save the optimization results?",
                QMessageBox.Yes | QMessageBox.No
            )
            ...
            self.status_bar.showMessage(f"Error running optimization: {str(e)}")
            self._log('error', f"Error running optimization: {str(e)}")
            QMessageBox.critical(self, "Error", f"Error running optimization:\n{str(e)}")
    ...
    def update_optimization_results(self):
        ...
        all_results = self.optimization_results.get('all_results', [])
        meta = self.optimization_results.get('metadata', {})
        param_analysis = self.optimization_results.get('parameter_analysis', {})
        
        # Format optimization results text
        results_text = f"""
OPTIMIZATION RESULTS SUMMARY
===========================

Optimization Method: {meta.get('optimizer_type', 'Grid Search')}
...
"""
        ...
        # Add parameter analysis insights if available
        if param_analysis and 'insights' in param_analysis:
            results_text += "\nKey Insights:\n"
            for insight in param_analysis['insights']:
        ...
        if all_results:
            ...
            sorted_results = sorted(
                all_results, 
                key=lambda x: x['performance'].get(metric, 0) if x.get('performance') else 0, 
                reverse=True
            )
            
            # Show top 5 combinations
            results_text += "\nTop 5 Parameter Combinations:\n"
            results_text += f"{'Rank':<4} {'ATR':<5} {'Factor':<8} {'Buffer':<8} {'Stop':<6} {'Trades':<8} {'Win Rate':<10} {'P.Factor':<10} {'Return':<8}\n"
            results_text += "-" * 70 + "\n"
            ...
            for i, result in enumerate(sorted_results[:5]):
                ...
                results_text += f"{i+1:<4} {params.get('atr_length', 'N/A'):<5} {params.get('factor', 'N/A'):<8.2f} {params.get('buffer_multiplier', 'N/A'):<8.2f} {params.get('hard_stop_distance', 'N/A'):<6} {perf.get('trade_count', 0):<8} {perf.get('win_rate', 0)*100:<10.2f}% {perf.get('profit_factor', 0):<10.2f} {perf.get('total_profit_pct', 0):<8.2f}%\n"
        
        self.optimization_results_text.setText(results_text)
