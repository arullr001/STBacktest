...
class OptimizationManager:
    ...
    def run_all_optimizations(self):
        ...
        for method_name, opt_method in optimization_methods:
            try:
                self.logger.info(f"Starting {method_name} optimization")
                result = opt_method()
                ...
                if result and hasattr(result, 'fun'):
                    ...
                    if score < best_score:
                        ...
                        self.best_parameters = {
                            'period': int(round(result.x[0])),
                            'multiplier': float(result.x[1]),
                            'price_range': float(result.x[2]),
                            'method': method_name,
                            'score': -score  # Convert back to profit
                        }
                        self.logger.info(f"New best result found using {method_name} optimization")
                        self.logger.info(f"Parameters: {self.best_parameters}")
            except Exception as e:
                self.logger.error(f"{method_name} optimization failed: {str(e)}")
                continue

        if best_result is None:
            self.logger.warning("All optimization methods failed, using default parameters")
            self.best_parameters = {
                'period': 50,  # Default middle values
                'multiplier': 15,
                'price_range': 1.5,
                'method': 'default',
                'score': 0
    ...
    def get_best_parameters(self):
        ...
        if not self.best_parameters and self.best_results:
            ...
            if best_result:
                self.best_parameters = {
                    'period': int(round(best_result.x[0])),  # Ensure integer
                    'multiplier': float(best_result.x[1]),
                    'price_range': float(best_result.x[2]),
                    'method': best_method,
                    'score': -best_result.fun
                }
        
        return self.best_parameters

    def objective_function(self, x):
        """Objective function for optimization with improved error handling"""
        try:
            # Ensure proper type conversion
            period = max(PERIOD_MIN, min(PERIOD_MAX, int(round(float(x[0])))))
            multiplier = max(MULTIPLIER_MIN, min(MULTIPLIER_MAX, float(x[1])))
            price_range = max(0.5, min(3.01, float(x[2])))
            ...
            return float('inf')  # Return worst possible score for invalid results

        except (ValueError, TypeError, ZeroDivisionError) as e:
            self.logger.error(f"Error in objective function: {str(e)}")
            return float('inf')
        except Exception as e:
            self.logger.error(f"Unexpected error in objective function: {str(e)}")
            return float('inf')

    def bayesian_optimization(self):
        """Perform Bayesian optimization with improved error handling"""
        try:
            self.logger.info("Starting Bayesian optimization")
            
            # Define bounds for skopt
            space = self.opt_space.get_space()
            
            # Add noise parameter to handle numerical instabilities
            result = gp_minimize(
            ...
            if result.fun != float('inf'):
                ...
                self.logger.error("Bayesian optimization failed to find valid solution")
                return None
    ...
    def evolutionary_optimization(self):
        """Perform evolutionary optimization with improved error handling"""
        try:
            ...
            # Add constraints to ensure period is integer
            def constraint_period_int(x):
                return abs(x[0] - round(x[0]))  # Should be 0 for integers
            
            constraints = ({'type': 'eq', 'fun': constraint_period_int})
            ...
            if result.fun != float('inf'):
                ...
                self.logger.error("Evolutionary optimization failed to find valid solution")
                return None
...
class ResultsManager:
    ...
    def save_detailed_results(self, final_results_df, original_data):
        ...
        try:
            ...
            results_file = os.path.join(self.dir_manager.final_results_dir, 'full_results.csv')
            final_results_df.to_csv(results_file, index=False)

            # Create visualization plots
            self.create_visualization_plots(final_results_df, original_data)

        except Exception as e:
            self.processing_logger.error(f"Error saving detailed results: {str(e)}")

    def create_visualization_plots(self, df, original_data):
        """Creates visualization plots for the results"""
        try:
            plt.style.use('seaborn')
            
            # Create plots directory
            plots_dir = os.path.join(self.dir_manager.final_results_dir, 'plots')
            os.makedirs(plots_dir, exist_ok=True)

            # 1. Profit Distribution
            plt.figure(figsize=(10, 6))
            plt.hist(df['avg_profit'], bins=50, edgecolor='black')
            plt.title('Distribution of Average Profit')
            ...
            axes[0].scatter(df['period'], df['avg_profit'], alpha=0.5)
            axes[0].set_title('Period vs Profit')
            axes[0].set_xlabel('Period')
            axes[0].set_ylabel('Average Profit (%)')

            axes[1].scatter(df['multiplier'], df['avg_profit'], alpha=0.5)
            axes[1].set_title('Multiplier vs Profit')
            axes[1].set_xlabel('Multiplier')
            axes[1].set_ylabel('Average Profit (%)')

            axes[2].scatter(df['price_range_1'], df['avg_profit'], alpha=0.5)
            axes[2].set_title('Price Range vs Profit')
            axes[2].set_xlabel('Price Range')
            axes[2].set_ylabel('Average Profit (%)')

            plt.tight_layout()
            plt.savefig(os.path.join(plots_dir, 'parameter_impact.png'))
            plt.close()

        except Exception as e:
            self.processing_logger.error(f"Error creating visualization plots: {str(e)}")
    ...
    def process_optimized_batch(self, df, optimization_manager, batch_size, max_workers=4):
        ...
        try:
            ...
            if not self.best_params:
                self.optimization_logger.error("No valid parameters found during optimization")
                ...
                self.optimization_logger.info("Using default parameters as fallback")
...
def calculate_profit_factor(trades):
    ...
    gross_profit = sum(trade['points'] for trade in trades if trade['points'] > 0)
    gross_loss = abs(sum(trade['points'] for trade in trades if trade['points'] < 0))
...
def calculate_sharpe_ratio(trades, risk_free_rate=0.02):
    ...
    returns = [trade['points'] for trade in trades]
...
def process_parameter_combination(df, params):
    """Process a single parameter combination with improved error handling"""
    try:
        # Validate and convert parameters
        period = int(round(float(params[0])))
        multiplier = float(params[1])
        price_range_1 = float(params[2])
        long_target = float(params[3])
        short_target = float(params[4])
        
        # Validate parameter ranges
        if not (PERIOD_MIN <= period <= PERIOD_MAX):
            raise ValueError(f"Period {period} out of valid range [{PERIOD_MIN}, {PERIOD_MAX}]")
        if not (MULTIPLIER_MIN <= multiplier <= MULTIPLIER_MAX):
            raise ValueError(f"Multiplier {multiplier} out of valid range [{MULTIPLIER_MIN}, {MULTIPLIER_MAX}]")
        if not (0.5 <= price_range_1 <= 3.01):
            raise ValueError(f"Price range {price_range_1} out of valid range [0.5, 3.01]")
        ...
        if result and isinstance(result.get('trades'), list):
            return {
                ...
                'avg_profit': result.get('total_profit', 0) / len(result['trades']) if result['trades'] else 0,
                'max_drawdown': calculate_max_drawdown(result['trades']),
        ...
        logging.getLogger('processing_errors').error(
            f"Error processing combination (period={params[0]}, mult={params[1]}, "
            f"range={params[2]}): {str(e)}"
        )
        return None
    except Exception as e:
        logging.getLogger('processing_errors').error(
            f"Unexpected error processing combination: {str(e)}"
        )
        return None	
