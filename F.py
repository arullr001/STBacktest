...
def main():
    ...
    """
    try:
        CURRENT_UTC = "2025-06-12 12:38:41"  # Updated timestamp
        CURRENT_USER = "arullr001"           # Updated user
        ...
        for file_path in selected_files:
            ...
            try:
                ...
                for batch_start in range(0, total_combinations, batch_size):
                    ...
                    current_batch = param_combinations[batch_start:batch_end]

                    print(f"\nProcessing batch {batch_processor.current_batch + 1} "
                          f"({batch_start + 1}-{batch_end} of {total_combinations})")
                    try:
                        batch_results = []
                ...
                if all_results:
                    results_data = [{
                        ...
                        'max_consecutive_wins': r['max_consecutive_wins'],
                        'max_consecutive_losses': r['max_consecutive_losses']
                    } for r in all_results if r['trade_count'] > 0]
                    ...
                    if results_data:
                        ...
                        print(final_results_df[['atr_length', 'factor', 'buffer_multiplier',
                                                ...
                                                'win_rate']].head().to_string(index=False))
                    else:
                        save_empty_results_file(file_path, dir_manager.final_results_dir,
                                                params, CURRENT_UTC, CURRENT_USER,
                                                total_combinations, reason="No parameter combinations generated profitable trades.")
                else:
                    save_empty_results_file(file_path, dir_manager.final_results_dir,
                                            params, CURRENT_UTC, CURRENT_USER,
                                            total_combinations, reason="No trades were generated with any parameter combination.")
