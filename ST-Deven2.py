...
class OptimizationManager:
    ...
    # Remove the @use_named_args decorator and modify the function
    def objective_function(self, x):
        """Objective function for optimization"""
        try:
            # Convert array parameters to their proper types
            period = int(x[0])
            multiplier = float(x[1])
            price_range = float(x[2])
    ...
    try:
        ...
        if result:
            return {
                ...
                'avg_profit': result['total_profit'] / len(result['trades']) if result['trades'] else 0,
