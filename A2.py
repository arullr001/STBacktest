...
BATCH_SIZE = 1000
CURRENT_UTC = "2025-05-28 03:21:10"  # Updated timestamp
...
CURRENT_UTC = "2025-05-28 03:23:57"
...
def calculate_supertrend_pine_cuda_kernel(high, low, close, atr_period, factor, buffer_distance, hardstop_distance, 
                                         up, dn, direction, supertrend, up_trend_buffer, down_trend_buffer, up_trend_hardstop, down_trend_hardstop):
    """
    CUDA kernel for Pine Script SuperTrend calculation with buffers and hard stops
    REVISED to match Pine Script convention:
    - direction < 0 for uptrend
    - direction > 0 for downtrend
    """
    i = cuda.grid(1)
    if i >= len(close):
        return
    ...
    if i == 0:
        up[i] = up_basic
        dn[i] = dn_basic
        direction[i] = 1  # Start with downtrend (direction > 0)
    else:
        # Calculate up
        if close[i-1] <= up[i-1]:
        ...
        # Determine trend direction - REVISED to match Pine Script
        if close[i] > dn[i-1]:
            direction[i] = -1  # Uptrend (direction < 0)
        elif close[i] < up[i-1]:
            direction[i] = 1  # Downtrend (direction > 0)
        else:
            direction[i] = direction[i-1]  # Maintain previous trend

    # Determine supertrend value based on direction
    if direction[i] < 0:  # Uptrend
        supertrend[i] = up[i]
    else:  # Downtrend
        supertrend[i] = dn[i]

    # Calculate buffer lines based on trend - REVISED to match Pine Script
    if direction[i] < 0:  # Uptrend
        up_trend_buffer[i] = supertrend[i] + buffer_distance
        ...
        down_trend_buffer[i] = supertrend[i] - buffer_distance
...
def calculate_supertrend_pine_gpu(df, atr_period, factor, buffer_distance, hardstop_distance):
    ...
    direction_gpu = cuda.device_array(n, dtype=np.float64)
    supertrend_gpu = cuda.device_array(n, dtype=np.float64)
    ...
    calculate_supertrend_pine_cuda_kernel[blocks_per_grid, THREADS_PER_BLOCK](
        ...
        up_gpu, dn_gpu, direction_gpu, supertrend_gpu, up_trend_buffer_gpu, down_trend_buffer_gpu, 
    ...
    supertrend = supertrend_gpu.copy_to_host()
    ...
    # Generate Pine Script signals - REVISED to match Pine Script
    df['buy_signal'] = (df['direction'] < 0) & (df['close'] <= df['up_trend_buffer']) & (df['close'] >= df['supertrend'])
    df['sell_signal'] = (df['direction'] > 0) & (df['close'] >= df['down_trend_buffer']) & (df['close'] <= df['supertrend'])

    # Clean up GPU memory
    try:
        del (high_gpu, low_gpu, close_gpu, up_gpu, dn_gpu, direction_gpu, supertrend_gpu,
...
def calculate_supertrend_pine_numba(high, low, close, atr_period, factor, buffer_distance, hardstop_distance):
    ...
    for i in range(n):
        ...
        if i == 0:
            ...
            dn[i] = dn_basic
            direction[i] = 1  # Start with downtrend (direction > 0)
            supertrend[i] = dn[i]  # Initial supertrend is based on trend
        else:
            ...
            # Update direction - REVISED to match Pine Script
            if close[i] > dn[i-1]:
                direction[i] = -1  # Uptrend (direction < 0)
            elif close[i] < up[i-1]:
                direction[i] = 1   # Downtrend (direction > 0)
        ...
        # Calculate buffer and hard stop lines based on current trend - REVISED to match Pine Script
        if direction[i] < 0:  # Uptrend
            up_trend_buffer[i] = supertrend[i] + buffer_distance
    ...
    return up, dn, direction, supertrend, up_trend_buffer, down_trend_buffer, up_trend_hardstop, down_trend_hardstop
...
def calculate_supertrend_pine_cpu(df, atr_period, factor, buffer_distance, hardstop_distance):
    ...
    up, dn, direction, supertrend, up_trend_buffer, down_trend_buffer, up_trend_hardstop, down_trend_hardstop = calculate_supertrend_pine_numba(
    ...
    try:
        if cuda.is_available():
            try:
                ...
                data_size = len(df) * 8 * 8  # 8 arrays: up, dn, direction, supertrend, up_buffer, down_buffer, up_hardstop, down_hardstop
...
# Updated timestamp constant
CURRENT_UTC = "2025-05-28 03:26:36"
CURRENT_USER = "arullr001"
...
def backtest_supertrend_pine(df, atr_period, factor, buffer_distance, hardstop_distance, enable_target=True, long_rr=2, short_rr=2):
    ...
    REVISED to match Pine Script convention and logic
    """
    ...
    for i in range(1, len(st_df)):
        ...
        if in_long:
            ...
            # Check for target hit (if enabled) - highest priority
            if enable_target and current_high >= target_price:
                exit_condition = True
                exit_price = target_price
                exit_reason = "Buy Target"
            
            # Check trend change or close < supertrend (second priority)
            elif current_price < current_supertrend or (not enable_target and trend_changed):
                exit_condition = True
                exit_price = current_price
                exit_reason = "Buy SL" if current_price < current_supertrend else "Buy Close Profit"
            
            # Check hard stop last (lowest priority)
            elif current_low <= up_hardstop[i] and up_hardstop[i] > 0:
                exit_condition = True
                exit_price = up_hardstop[i]
                exit_reason = "Buy Hard Stop"
        ...
        if in_short:
            ...
            # Check for target hit (if enabled) - highest priority
            if enable_target and current_low <= target_price:
                exit_condition = True
                exit_price = target_price
                exit_reason = "Sell Target"
            
            # Check trend change or close > supertrend (second priority)
            elif current_price > current_supertrend or (not enable_target and trend_changed):
                exit_condition = True
                exit_price = current_price
                exit_reason = "Sell SL" if current_price > current_supertrend else "Sell Close Profit"
            
            # Check hard stop last (lowest priority)
            elif current_high >= down_hardstop[i] and down_hardstop[i] > 0:
                exit_condition = True
                exit_price = down_hardstop[i]
                exit_reason = "Sell Hard Stop"
        ...
        if not in_long and not in_short:
            # Buy signal condition - REVISED to match Pine Script
            if buy_signals[i]:
                trade_number += 1
                in_long = True
            ...
            # Sell signal condition - REVISED to match Pine Script
            elif sell_signals[i]:
                trade_number += 1
                in_short = True
                entry_price = current_price
...
# Updated timestamp constant
CURRENT_UTC = "2025-05-28 03:29:09"
CURRENT_USER = "arullr001"
...
def main():
    ...
    try:
        ...
        print(f"{'='*80}")
        print(f"Version: Aligned with Pine Script Logic")
        print(f"Timestamp: {CURRENT_UTC}")
        print(f"User: {CURRENT_USER}")
        print(f"{'='*80}")
